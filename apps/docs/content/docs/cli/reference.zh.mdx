...
如果当前目录是 git checkout，且 `git remote get-url origin` 能唯一匹配当前 workspace 某个项目资源的 `resource_ref.url`，`issue create` 会默认把 issue 绑定到该项目。显式 `--project <id>` 优先；多个项目匹配同一 repo 时不会猜测，请手动传 `--project`。
...
`issue create` 在 git checkout 内也会尝试根据 `origin` remote 自动匹配项目资源；只有唯一匹配时才自动绑定。需要覆盖或消除歧义时，直接传 `--project <project-id>`。
