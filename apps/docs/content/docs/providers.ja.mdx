...
description: Multica は 19 個の AI コーディングツールをサポートしています。すべて同じインターフェースを実装していますが、機能の詳細は大きく異なります。
---

import { Callout } from "fumadocs-ui/components/callout";

Multica は **19 個の AI コーディングツール**を標準でサポートしています。これらはすべて同じインターフェース（キューへの投入、ディスパッチ、実行、結果の返却）を実装しているため、同じ Multica ボードからどれでも動かすことができます。**しかし機能の詳細は大きく異なります**: セッション再開が実際に動作するか、MCP をサポートするか、スキルファイルがどこに置かれるか、モデルをどう選択するか。このページがその完全な対応表です。
...
| **OMP** | Oh My Pi | ✅ (`--resume <id>`) | ❌ | `.omp/skills/` | 動的探索（`omp models --no-extensions --json`） |
...
| **MiniMax Code** | MiniMax | —（0.1.2 はセッション読み込みを通知しない） | ✅ | `.minimax/skills/` | ランタイムで管理 |
...
OpenAI が提供します。JSON-RPC 2.0 を使用し、ステートフルな能力がより強く、よりきめ細かい承認メカニズム（`exec_command` および `patch_apply` に対する手動承認）を備えています。MCP 構成はタスクごとの `$CODEX_HOME/config.toml` に書き込まれます。**セッション再開は動作します** — Multica は Codex app-server の `thread/resume` で再開します。保存済み thread が見つからない、または古い場合は、新しい thread にフォールバックしてタスクを続行します。 MiniMax Code 0.1.2 は ACP でセッション読み込み機能を通知しないため、後続の実行では新しい MCode セッションを開始します。
...
[Oh My Pi](https://omp.sh) は独立したコーディングエージェントであり、**Pi の別名ではありません**。Multica は `omp -p --mode json --session-dir <dir>` を非対話モードで実行し、Pi 互換の JSON イベントストリームを読み取ります。イベントで返る文字列の session ID は、次のタスクで `--resume <id>` に渡して再開します。モデルと対応する thinking level は `omp models --no-extensions --json` から動的に探索され、選択された model selector は `--model`、thinking level は `--thinking` で渡されます。割り当てられたワークスペーススキルは、OMP のネイティブ探索用に `.omp/skills/` へ書き込まれます。

OMP を有効にするには、最初に OMP をインストールして認証し、daemon の `PATH` から `omp` を見つけられるようにするか、`MULTICA_OMP_PATH` に絶対実行パスを設定します。daemon を再起動または更新してランタイムを再検出します。OMP selector を `MULTICA_OMP_MODEL` に設定すると daemon 全体の既定モデルになり、個別のエージェントでは動的に検出された selector を選択できます。OMP は現在、エージェントの `mcp_config` を無視します。
...
MiniMax 提供の ACP ネイティブなコーディングエージェント（`mcode` CLI）です。MCP 構成は ACP `mcpServers` として渡され、スキルは `.minimax/skills/` に書き込まれてネイティブに発見されます。モデル選択はランタイム側で管理されるため、選択欄は操作できず「ランタイムで管理」と表示されます。MiniMax Code 0.1.2 は ACP でセッション読み込み機能を通知しないため、後続の実行では新しい MCode セッションを開始します。
...
**19 個のツールのうち、`mcp_config` を実際に消費するのは 12 個です: Claude Code、CodeBuddy、Codex、Cursor、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Qoder、Trae CLI、Grok**。残りの 5 個（Antigravity、Copilot、DevEco Code、Pi、OMP）はこのフィールドを受け取りますが、**無視します** — エラーも警告もなく、構成はただ効果を発揮しません。
...
- [エージェントランタイムのインストール](/install-agent-runtime) — サポートされる 19 個のツールそれぞれのインストールと認証
