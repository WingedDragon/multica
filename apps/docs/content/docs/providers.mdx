...
**Of the 18 tools, thirteen consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Qwen Code, Trae, and Grok**. The other five (Antigravity, Copilot, DevEco Code, Pi, and OMP) accept the field but **ignore it** — no error, no warning, the config just has no effect.
...
- [Install an agent runtime](/install-agent-runtime) — installation and authentication for each of the 18 supported tools
