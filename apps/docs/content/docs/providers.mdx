...
| **OMP** | Oh My Pi | ✅ (`--resume <id>`) | ❌ | `.pi/skills/` | Dynamic discovery (`omp models --json`) |
...
[Oh My Pi](https://omp.sh) is a standalone coding agent, **not a Pi alias**. Multica launches it non-interactively as `omp -p --mode json --session-dir <dir>`, reads its Pi-compatible JSON event stream, and saves the emitted string session ID for `--resume <id>` on the next task. Models and their supported thinking levels are discovered from `omp models --json`; Multica sends the selected model selector with `--model` and the selected level with `--thinking`. Assigned workspace skills are written to `.pi/skills/` for OMP's native discovery.
