---
title: AI 编程工具对照
description: Multica 支持 17 款 AI 编程工具；它们实现同一套接口，但能力细节差异很大。
---

import { Callout } from "fumadocs-ui/components/callout";

Multica 内置支持 **17 款 AI 编程工具**。它们都实现了同一套接口——排队、派发、执行、结果回传，所以你可以从 Multica 的同一个看板上指挥任意一款。**但它们在能力细节上差异很大**：会话恢复是否真用、是否支持 MCP、skill 文件该放在哪里、模型怎么选。这一页是完整对照。
...
**17 款工具里有 12 款实际消费 `mcp_config`：Claude Code、CodeBuddy、Codex、Cursor、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Qoder、Trae CLI、Grok**。其他 5 款（Antigravity、Copilot、DevEco Code、Pi、OMP）会接收这个字段但**忽略**——不报错、不警告，只是配置不生效。
