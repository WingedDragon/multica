...
| **OMP** | Oh My Pi | ✅（`--resume <id>`）| ❌ | `.pi/skills/` | 动态发现（`omp models --json`）|
...
[Oh My Pi](https://omp.sh) 是一款独立的编程智能体，**不是 Pi 的别名**。Multica 以非交互方式执行 `omp -p --mode json --session-dir <目录>`，读取其兼容 Pi 的 JSON 事件流，并保存事件返回的字符串 session ID，以便下一次任务通过 `--resume <id>` 续接。模型及其支持的思考强度从 `omp models --json` 动态发现；选中的模型 selector 通过 `--model` 传入，思考强度通过 `--thinking` 传入。分配给工作区的 skill 写入 `.pi/skills/`，由 OMP 原生发现。
