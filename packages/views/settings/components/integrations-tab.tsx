...
import { gitlabConfigOptions } from "@multica/core/gitlab";
...
// Integrations is the directory for third-party connections. Each provider
// owns its status summary, setup flow, and detail view.
export function IntegrationsTab() {
  ...
  const gitlab = useQuery({
    ...gitlabConfigOptions(wsId),
    enabled: canView,
    select: (data) => data.projects.length > 0,
  });
  const octo = useQuery({
  ...
  const groups: {
  ...
  }[] = [
    {
      ...
      entries: [
        ...
        {
          id: "gitlab",
          label: t(($) => $.page.tabs.gitlab),
          description: t(($) => $.gitlab.page_description),
          icon: <GitLabMark className="size-5" />,
          content: <GitLabTab />,
          state: gitlab,
        },
        ...(vcsAvailable
    ...
    },
    {
      ...
      entries: [
        ...
        },
        {
          id: "octo",
          label: "Octo",
          description: "Connect Octo IM bots to agents.",
