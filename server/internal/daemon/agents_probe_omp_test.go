...
func TestProbeAgentCLIs_DiscoversPiAndOmpSeparately(t *testing.T) {
	...
	if ompDisplayName != "OMP" {
		t.Errorf("omp display name = %q, want %q", ompDisplayName, "OMP")
	}
...
func TestRegisterRuntimes_PiAndOmpBothReachTheServer(t *testing.T) {
	...
	if got := fx.registeredNameFor("ws-1", "omp"); got != "OMP" {
		t.Errorf("registered omp name = %q, want %q", got, "OMP")
	}
