...
//
// Pi is the exception: its opaque session id is an absolute JSONL path under
// ~/.multica/pi-sessions, and the backend passes that path directly to
// --session. Native OMP is also independent of the task cwd, but its CLI uses
// the shared ~/.multica/omp-sessions directory with opaque --resume ids rather
// than Pi's --session path. Both stores survive workdir changes, so neither
// provider should bind a resume to workdir reuse.
...
func gateResumeToReachableSession(task *Task, taskCtx *execenv.TaskContextForEnv, provider, envWorkDir string, sessionHomeReachable, refusesMissingSessionCwd bool, taskLog *slog.Logger) bool {
	var reachable bool
	switch {
	case providerUsesPiSessionFile(provider):
		reachable = piSessionResumable(task.PriorSessionID, refusesMissingSessionCwd)
	case provider == "omp":
		// OMP stores transcripts in its shared session directory and resumes
		// by opaque id, so a task workdir change does not make the id stale.
...
//   - omp keeps its transcript in the native shared session directory and
//     resumes it by opaque id, so a missing recorded cwd does not invalidate
//     the session.
//
// So the check must not be applied family-wide: doing that drops sessions OMP
// would have resumed cleanly, which is exactly the continuity loss #7760 set
// out to fix.
